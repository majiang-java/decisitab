...
<script type="text/javascript">
	...
	function makeJar(){
		...
		$.ajax({
			type : "POST",
			url : "ruleProdController.do?makeJar",
			data : data1,	
			success : function(data) {
				//var message="接口调用结果："+JSON.parse(JSON.parse(data).obj).status+"\n"+"日志信息："+JSON.parse(JSON.parse(data).obj).msg;
				console.info(JSON.parse(data).obj);
...
<body style="overflow-y: hidden" scroll="no">
  <t:formvalidone formid="formobj" layout="div" usePlugin="password"  dialog="true"   refresh="true" action="ruleProdController.do?datagrid">  
	...
	<div class="form"><label class="Validform_label" style="width:70px"> POM配置： </label> <textarea cols="88" name="descp" rows="18" >${RuleProdEntity.descp}</textarea></div>
