...
public class ScoreModelObj {
     ...
     */
    public List<Serializable> getScoreObjs(Object object, List<Serializable> objs, LoanApplication loanApp) throws Exception {
         for (Class<?> classType = object.getClass(); classType != Object.class; classType = classType.getSuperclass()) {
            ...
            for (Field fd : fdAttr) {
                ...
                if (!((fieldName.equals("serialVersionUID") || (fieldName.equals("name"))))) {
                    ...
                    setObjList(objs, fieldName, value,loanApp);
                }
            }
     ...
     */
    private List<Serializable> getScoreMapObjs(Object object, List<Serializable> objs, LoanApplication loanApp) {
     ...
     */
    private void setObjList(List<Serializable> objs, String fieldName, Object value, LoanApplication loanApp) {
        ...
        try {
            ...
            if (!valueCode.equals("")) {
	            ...
	            if(loanApp.getRejectReason()!=null){
	            	loanApp.getRejectReason().add(fieldName);
	            }
	            objs.add(scoreCard);
            }
    ...
    public LoanApplication getPricingObj(Object object, LoanApplication loanApp) throws IOException {
        ...
        if (object == null || loanApp == null) {
            ...
            if (object != null && object instanceof Map) {
                objs = getScoreMapObjs(object, objs,loanApp);
            } else {
                objs = getScoreObjs(object, objs,loanApp);
            }
        } catch (Exception e) {
            logger.error(e);
            e.printStackTrace();
        ...
        if ((objs != null) && (objs.size() > 0)) {
            ...
            ksession.execute(objs);            
            objs.clear();
            Iterator<?> it = loanApp.getRejectReason().iterator();
            while (it.hasNext()) {
	            ScoreCard scoreCard = new ScoreCard();
	            scoreCard.setVarName((String) it.next());
	            scoreCard.setVarValue(-99);
	            objs.add(scoreCard);
            }
            loanApp.getRejectReason().clear();
            objs.add(loanApp);
            ksession.execute(objs);
            return loanApp;
        } else {
