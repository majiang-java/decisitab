...
public class DataGridTag extends TagSupport {
	...
	public void setColumn(String title, String field, Integer width, Integer showLen, String rowspan, String colspan,
			String align, boolean sortable, boolean checkbox, String formatter, String formatterjs, boolean hidden,
			String replace, String treefield, boolean image, String imageSize, boolean query, String url,
			String funname, String arg, String queryMode, String dictionary, boolean popup, boolean frozenColumn,
			String extend, String style, String downloadName, boolean isAuto, String extendParams, String editor,
			String defaultVal) {
		...
		// author：xugj--start--date:2016年5月11日 for:TASK #1080 【UI标签改造】t:dgCol
		// 显示内容长度控制 -->
		dataGridColumn.setShowLen(showLen);
		// author：xugj--end---date:2016年5月11日 for:TASK #1080 【UI标签改造】t:dgCol
		// 显示内容长度控制 -->
		...
		if (!StringUtils.isBlank(dictionary) && (!popup)) {
			if (dictionary.contains(",")) {
				...
				if (dictionary.equals("orgCode")) {
					// System.out.println("....................................orgDepart_auto");
					String text = "";
					String value = "";
					systemService = ApplicationContextUtil.getContext().getBean(SystemService.class);
					List<TSDepart> departList = systemService.getList(TSDepart.class);
					for (TSDepart object : departList) {
						value += object.getOrgCode() + ",";
						text += object.getDepartname() + ",";
					}
					// System.out.println("...................:"+text);
					// System.out.println("...................:"+value);
					// System.out.println("...................:"+field);
					setColumn(field, text, value);
				} else if (dictionary.equals("kKnwldgId")) {
					String text = "";
					String value = "";
					knwldgLibService = ApplicationContextUtil.getContext().getBean(KnwldgLibServiceI.class);
					List<KnwldgLibEntity> knowList = knwldgLibService.getList(KnwldgLibEntity.class);
					for (KnwldgLibEntity object : knowList) {
						value += object.getId() + ",";
						text += object.getName() + ",";
					}
					// System.out.println("...................:"+text);
					...
					// System.out.println("...................:"+field);
					setColumn(field, text, value);
				} else if (dictionary.equals("orgId")) {
					...
					setColumn(field, text, value);
				} else if (dictionary.equals("prodId")) {
					// System.out.println("....................................prodId");
					String text = "";
					String value = "";
					ruleProdService = ApplicationContextUtil.getContext().getBean(RuleProdServiceI.class);
					List<RuleProdEntity> ruleProdList = ruleProdService.getList(RuleProdEntity.class);
					for (RuleProdEntity object : ruleProdList) {
						value += object.getId() + ",";
						text += object.getName() + ",";
					}
					setColumn(field, text, value);
				} else if (dictionary.equals("pckgId")) {
					// System.out.println("....................................pckgId");
					String text = "";
					String value = "";
					rulePckgService = ApplicationContextUtil.getContext().getBean(RulePckgServiceI.class);
					List<RulePckgEntity> rulePckgList = rulePckgService.getList(RulePckgEntity.class);
					for (RulePckgEntity object : rulePckgList) {
						value += object.getId() + ",";
						text += object.getAllName() + ",";
					}
					setColumn(field, text, value);
				} else {
					String text = "";
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			// -----author:jg_longjb----start-----date:20150408--------for:读取cookie主题样式
			// ace界面下table的输出
			...
			if (style.equals("easyui")) {
				...
				out.flush();
				// }
				// -----author:jg_longjb----end-----date:20150408--------for:读取cookie主题样式
				// ace界面下table的输出
			} else {
	...
	public StringBuffer end() {
		...
		if (columnList.size() > 0) {
			...
			// 增加form实现Form表单验证,此处避免reset时走验证，代码做了冗余
							...
							if (!StringUtil.isEmpty(col.getReplace())) {
								...
								for (String string : test) {
									...
									if (col.getDefaultVal() != null && col.getDefaultVal().trim().equals(value)) {
										sb.append("<option value =\"" + value + "\" selected=\"selected\">" + text
												+ "</option>");
									} else {
								...
								if (col.getDictionary().contains(",") && col.isPopup()) {
									...
									if (col.getDefaultVal() != null && !col.getDefaultVal().trim().equals("")) {
										sb.append("<input type=\"text\" name=\"" + col.getField().replaceAll("_", "\\.")
												...
												+ dic[1] + "','" + dic[0] + "');\" value=\"" + col.getDefaultVal()
												+ "\"/> ");
									} else {
										sb.append("<input type=\"text\" name=\"" + col.getField().replaceAll("_", "\\.")
												+ "\" style=\"width: 100px\" class=\"searchbox-inputtext\" value=\"\" onClick=\"inputClick(this,'"
									...
									for (Map<String, Object> map : list) {
										...
										if (col.getDefaultVal() != null
												&& col.getDefaultVal().trim().equals(map.get("field"))) {
									...
									sb.append("</select>");
								} else if (col.getDictionary().equals("orgCode")
										|| col.getDictionary().equals("orgId")) {
									sb.append("<input id=\"ccc\"  name=\"orgId\" value=\"\">");
									// systemService =
									// ApplicationContextUtil.getContext().getBean(
									// SystemService.class);
									//// List<TSDepart> departList =
									// systemService.getList(TSDepart.class);
									//
									...
									// cq.add();
									// List<TSDepart> departList =
									// systemService.getListByCriteriaQuery(cq,
									// false);

									// sb.append("<select
									// name=\""+col.getField().replaceAll("_","\\.")+"\"
									// WIDTH=\"100\" style=\"width: 134px\"> ");
									// sb.append(StringUtil.replaceAll("<option
									// value =\"\" >{0}</option>", "{0}",
									// MutiLangUtil.getMutiLangInstance().getLang("common.please.select")));
									//
									// if (departList != null) {
									// if(col.getDictionary().equals("orgCode")){
									// for (TSDepart type : departList) {
									// sb.append(" <option value=\""
									// + type.getOrgCode()
									// + "\">");
									// sb.append(MutiLangUtil.getMutiLangInstance().getLang(type.getDepartname()));
									// sb.append(" </option>");
									// }
									// }else{
									// for (TSDepart type : departList) {
								...
								} else if (col.getDictionary().equals("kKnwldgId")) {
									knwldgLibService = ApplicationContextUtil.getContext()
											.getBean(KnwldgLibServiceI.class);
									List<KnwldgLibEntity> knowList = null;
									String orgId = ResourceUtil.getSessionUserName().getCurrentDepart().getId();
									String username = ResourceUtil.getSessionUserName().getUserName();
									if (username.equalsIgnoreCase("admin")) {
										knowList = knwldgLibService.getList(KnwldgLibEntity.class);
									} else {
										try {
											knowList = knwldgLibService.findKnwldgLibEntityByOrgId(orgId);
									...
									if (knowList != null) {
										for (KnwldgLibEntity type : knowList) {
											if (StringUtil.isNotEmpty(type.getName())) {
												sb.append(" <option value=\"" + type.getId() + "\">");
												sb.append(MutiLangUtil.getMutiLangInstance().getLang(type.getName()));
												sb.append(" </option>");
											}
										}
									}
									sb.append("</select>");
									// System.out.println("..........................sb:"+sb);
								} else if (col.getDictionary().equals("prodId")) {
									ruleProdService = ApplicationContextUtil.getContext()
											.getBean(RuleProdServiceI.class);
									String orgId = ResourceUtil.getSessionUserName().getCurrentDepart().getId();
									List<RuleProdEntity> ruleProdList = null;
									String username = ResourceUtil.getSessionUserName().getUserName();
									if (username.equalsIgnoreCase("admin")) {
										ruleProdList = ruleProdService.getList(RuleProdEntity.class);
									} else {
										try {
											ruleProdList = ruleProdService.findRuleProdEntityByOrgId(orgId);
											System.out.println(
													"..................................>>" + ruleProdList.size());
										} catch (IfreException e) {
											// TODO Auto-generated catch block
									...
									if (ruleProdList != null) {
										for (RuleProdEntity type : ruleProdList) {
											if (StringUtil.isNotEmpty(type.getName())) {
												sb.append(" <option value=\"" + type.getId() + "\">");
												sb.append(MutiLangUtil.getMutiLangInstance().getLang(type.getName()));
												sb.append(" </option>");
									...
									sb.append("</select>");
									// System.out.println("..........................sb:"+sb);
								} else if (col.getDictionary().equals("pckgId")) {
									rulePckgService = ApplicationContextUtil.getContext()
											.getBean(RulePckgServiceI.class);
									String orgId = ResourceUtil.getSessionUserName().getCurrentDepart().getId();
									List<RulePckgEntity> rulePckgList = null;
									String username = ResourceUtil.getSessionUserName().getUserName();
									if (username.equalsIgnoreCase("admin")) {
										rulePckgList = rulePckgService.getList(RulePckgEntity.class);
									} else {
										try {
											rulePckgList = rulePckgService.findRulePckgEntityByOrgId(orgId);
										} catch (IfreException e) {
											// TODO Auto-generated catch block
											e.printStackTrace();
										}
									...
									if (rulePckgList != null) {
										for (RulePckgEntity type : rulePckgList) {
											if (StringUtil.isNotEmpty(type.getAllName())) {
												sb.append(" <option value=\"" + type.getId() + "\">");
												sb.append(
														MutiLangUtil.getMutiLangInstance().getLang(type.getAllName()));
												sb.append(" </option>");
											}
										}
									}
									...
									if (types != null) {
										for (TSType type : types) {
											...
											if (col.getDefaultVal() != null
													&& col.getDefaultVal().trim().equals(type.getTypecode())) {
								...
								if (col.getDefaultVal() != null && !col.getDefaultVal().trim().equals("")) {
									sb.append(
											"<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""
													...
													+ extendAttribute(col.getExtend())
													+ "  class=\"inuptxt\" style=\"width: 100px\" value=\""
													+ col.getDefaultVal() + "\" />");
								} else {
									sb.append(
											"<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""
								...
								// "+extendAttribute(col.getExtend())+"
								// class=\"inuptxt\" style=\"width: 100px\"
								// value="+col.getDefaultVal()==null?"":"\""+col.getDefaultVal()+"\""+"/>");
		...
		for (DataGridUrl dataGridUrl : list) {
			...
			if (OptTypeDirection.OpenWin.equals(dataGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'#\' onclick=openwindow('" + dataGridUrl.getTitle() + "','" + url + "','"
						+ name + "'," + dataGridUrl.getWidth() + "," + dataGridUrl.getHeight() + ")" + style.toString()
						+ ">\";");
		...
		for (DataGridColumn column : columnList) {
			...
			if ((column.isFrozenColumn() && frozen == 0) || (!column.isFrozenColumn() && frozen == 1)) {
				...
				if (column.getFormatterjs() != null) {
					...
					// author:xugj-----start-----date:20160512 ---- for: TASK
					// #1080 【UI标签改造】t:dgCol 显示内容长度控制
					...
					// author:xugj-----end-----date:20160512 ---- for: TASK
					// #1080 【UI标签改造】t:dgCol 显示内容长度控制
						...
						for (ColumnValue columnValue : columnValueList) {
							if (columnValue.getName().equals(column.getField())) {
								...
								for (int j = 0; j < value.length; j++) {
									if (value[j].equals("\"\"")) {
										testString += "if(value==\"\"){return \'" + text[j] + "\';}";
									} else {
	...
	private String getAutoSpan(String filed, String extend) {
		...
		nsb.append("$(\"#" + getEntityName() + "_" + id + "\").val(row['" + filed + "']);}); });").append("</script>")
				.append("<input class=\"inuptxt\" type=\"text\" id=\"" + getEntityName() + "_" + id + "\" name=\""
						+ filed + "\"  " + extend + StringUtil.replace(" nullmsg=\"\" errormsg=\"{0}\"/>", "{0}",
								MutiLangUtil.getMutiLangInstance().getLang("input.error")));
		return nsb.toString();
	}
	...
	// -----author:jg_longjb----start-----date:20150408--------for:新增ace
	// 界面下的button class样式
