...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	public DataGridReturn getDataGridReturn(final CriteriaQuery cq,
		...
		List list=null;
		System.out.println(cq.getEntityClass());
		String entityName=cq.getEntityClass().toString();
		list= criteria.list();
	 ...
	 * 返回easyui datagrid DataGridReturn模型对象
	 */
	public DataGridReturn getDataGridAReturn(final CriteriaQuery cq,
			final boolean isOffset,String sql) {
		String entityName=cq.getEntityClass().toString();   
		final int allCounts =getListCount( entityName,sql) ;
		int pageSize = cq.getPageSize();// 每页显示数
		int curPageNO = PagerUtil.getcurPageNo(allCounts, cq.getCurPage(),
				pageSize);// 当前页
		List list=null;
		System.out.println(cq.getEntityClass());
		int offset = PagerUtil.getOffset(allCounts, curPageNO, pageSize);
		list=getList(offset,cq,entityName,sql);
		cq.getDataGrid().setResults(list);
		cq.getDataGrid().setTotal(allCounts);
		return new DataGridReturn(allCounts, list);
	}
	
	private int getListCount(String entityName,String asql){
		Map<String,TSDataRule> map = HqlGenerateUtil.getRuleMap();
		String sql = "";
		String middlesql="";
		int count = 0;
		middlesql=getMiddleSql(entityName);
		if (entityName.contains("BizObjEntity")) {			
			sql = "select count(ob.id) from BizObjEntity ob,RuleProdEntity pr,KnwldgLibEntity kn where ob.prodId=pr.id  and pr.kknwldgId=kn.id and kn.status=1";
		} else if (entityName.contains("RuleProdEntity")) {
			sql = "select count(pr.id) FROM RuleProdEntity pr,KnwldgLibEntity kn WHERE pr.kknwldgId=kn.id and kn.status=1";
		} else if (entityName.contains("RulePckgEntity")) {
			sql = "select count(pc.id) from KnwldgLibEntity kn,RulePckgEntity pc,RuleProdEntity pr where pr.id=pc.prodId and pr.kknwldgId=kn.id and kn.status=1";
		} else if (entityName.contains("BrmsRuleTableEntity")) {
			sql = "select count(tb.id) from KnwldgLibEntity kn,BrmsRuleTableEntity tb where tb.knowId=kn.id and kn.status=1";
		}
		sql = sql +middlesql+ asql;
	...
	private String getMiddleSql(String entityName){
		Collection<TSDataRule> dataRules = HqlGenerateUtil.getRuleMap().values();
		String middlesql="";
		String alise ="";
		if (entityName.contains("BizObjEntity")) {			
			alise="ob";
		...
		for(TSDataRule dataRule : dataRules){
			String a=HqlGenerateUtil.converRuleValue(dataRule.getRuleValue());			
			if(dataRule.getRuleConditions().equals("IN")){
				middlesql=" and "+alise+"."+dataRule.getRuleColumn()+" "+dataRule.getRuleConditions()+" ('";
			for(int i=0;i<a.split(",").length;i++){
				if(i==a.split(",").length-1)
				middlesql=middlesql+a.split(",")[i]+"'";
				else
				middlesql=middlesql+a.split(",")[i]+"','";	
			}
			middlesql=middlesql+")";
			}else{			
			middlesql=" and "+alise+"."+dataRule.getRuleColumn()+" "+dataRule.getRuleConditions()+" '"+HqlGenerateUtil.converRuleValue(dataRule.getRuleValue())+"'";
			}
		}		
		return middlesql;
	}
	...
	private String getOrderSql(String entityName,CriteriaQuery cq){
		String orderSql="";
	if (StringUtils.isNotBlank(cq.getDataGrid().getSort())) {
		if (entityName.contains("BizObjEntity")) {
			orderSql = " order by ob." + cq.getDataGrid().getSort() + " " + cq.getDataGrid().getOrder();
		} else if (entityName.contains("RulePckgEntity")) {
			orderSql = " order by pc." + cq.getDataGrid().getSort() + " " + cq.getDataGrid().getOrder();
		} else if (entityName.contains("RuleProdEntity")) {
			orderSql = " order by pr." + cq.getDataGrid().getSort() + " " + cq.getDataGrid().getOrder();
		}
	} else {
		if (entityName.contains("BizObjEntity")) {
			orderSql = " order by ob.id desc";
		} else if (entityName.contains("RulePckgEntity")) {
			orderSql = " order by pc.id desc";
		} else if (entityName.contains("RuleProdEntity")) {
			orderSql = " order by pr.id desc";
		} else if (entityName.contains("BrmsRuleTableEntity")) {
			orderSql = " order by tb.id desc";
		}
	}
	return orderSql;
	}
	private List getList(int start, CriteriaQuery cq, String entityName, String asql) {
		int size = cq.getPageSize();
		List list = null;
		...
		String orderSql = "";
		middlesql=getMiddleSql(entityName);
		orderSql=getOrderSql(entityName,cq);
		Map<String,TSDataRule> map = HqlGenerateUtil.getRuleMap();

		if (entityName.contains("BizObjEntity")) {
			sql = "select ob from BizObjEntity ob,RuleProdEntity pr,KnwldgLibEntity kn,RulePckgEntity pc where ob.prodId=pr.id and ob.pckgId=pc.id and pr.kknwldgId=kn.id and kn.status=1";
			sql = sql +asql + middlesql+ orderSql;
			System.out.println(sql);
			Query query = getSession().createQuery(sql).setFirstResult(start).setMaxResults(size);
			list = query.list();
			returnList = new ArrayList<BizObjEntity>();
		} else if (entityName.contains("RulePckgEntity")) {
			sql = "select pc from RuleProdEntity pr,KnwldgLibEntity kn,RulePckgEntity pc where pr.id=pc.prodId and pr.kknwldgId=kn.id and kn.status=1";
			sql = sql + asql +middlesql+ orderSql;
			Query query = getSession().createQuery(sql).setFirstResult(start).setMaxResults(size);
			list = query.list();
			returnList = new ArrayList<RulePckgEntity>();
			
		} else if (entityName.contains("RuleProdEntity")) {
			sql = "SELECT pr FROM RuleProdEntity pr,KnwldgLibEntity kn WHERE pr.kknwldgId=kn.id and kn.status=1 ";
			sql = sql + asql +middlesql+ orderSql;
			Query query = getSession().createQuery(sql).setFirstResult(start).setMaxResults(size);
			list = query.list();
			returnList = new ArrayList<RuleProdEntity>();
			
			
		} else if (entityName.contains("BrmsRuleTableEntity")) {
			sql = "SELECT tb FROM BrmsRuleTableEntity tb,KnwldgLibEntity kn WHERE tb.knowId=kn.id and kn.status=1 ";
			sql = sql + asql +middlesql + orderSql;
			logger.info("genSQL :" + sql);
			Query query = getSession().createQuery(sql).setFirstResult(start).setMaxResults(size);
			list = query.list();
			returnList = new ArrayList<BrmsRuleTableEntity>();
			returnList = list;
		}
		returnList = list;
		System.out.println(sql);
